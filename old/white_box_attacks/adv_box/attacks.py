...
class FGSM(object):
    ...
    def perturbation(self, X_i, y, epsilons=None):
        ...
        if self.mode == 'wf':
            ...
            X = utils_wf.add_perturbation(X,pert)

            "inverse the normalized data"
            X = utils_wf.inverse_normalize(X)

            "add round function to inversed data"
            X = utils_wf.round_data(X)
...
class DeepFool(object):
    ...
    def __init__(self,mode,x_box_min=-1,x_box_max=0,pert_box=0.3,model=None, num_classes=5):
        ...
        self.mode = mode
        self.pert_x_min,self.pert_x_max = -pert_box,pert_box
        self.x_box_min, self.x_box_max = x_box_min,x_box_max
        self.overshoot = 0.02
    ...
    def perturbation(self, X_input, y=None):
        ...
        while pert_label_i == pred_label and loop_i < self.max_iter:
            ...
            if self.mode == 'wf':
                "normalize data to [-1,1]"
                X_input = utils_wf.data_normalize(X_input)

                "clamp perturbation"
                noise_total_overshoot = torch.clamp(noise_total_overshoot, self.pert_box_min, self.pert_box_max)

                "add pert to x given the restrictions"
                pert_x = utils_wf.add_perturbation(X_input, noise_total_overshoot)

                "inverse the normalized data"
                pert_x = utils_wf.inverse_normalize(pert_x)

                "add round function to inversed data"
                pert_x = utils_wf.round_data(pert_x)

                "convert ndarray to torch.Tensor"
                pert_x = torch.Tensor(pert_x).to(self.device)
            elif self.mode == 'shs':
...
class LinfPGDAttack(object):
    ...
    def perturbation(self, X_nat, y):
        ...
        for i in range(self.k):
            ...
            if self.mode == 'wf':
                "normalize data to [-1,1]"
                X = utils_wf.data_normalize(X_var)
