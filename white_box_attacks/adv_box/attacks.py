...
from white_box_attacks import utils_wb as utils_advBox
import numpy as np
import torch
...
class FGSM(object):
    ...
    def __init__(self, mode,x_box_min=-1,x_box_max=0, model=None, epsilon=0.1):
        self.model = model
        self.epsilon = epsilon
        self.mode = mode
    ...
    def perturbation(self, X_i, y, epsilons=None):
        ...
        if self.mode == 'wf':
            X = utils_advBox.add_perturbation(X,pert)
        elif self.mode == 'shs':
            X += pert
...
class DeepFool(object):
    ...
    def __init__(self,mode,x_box_min=-1,x_box_max=0,model=None, num_classes=5):
        """
        num_classes: limits the number of classes to test against
        overshoot: used as a termination criterion to prevent vanishing updates
        max_iter: maximum number of iterations for deepfool
    ...
    def perturbation(self, X_input, y=None):
        ...
        while pert_label_i == pred_label and loop_i < self.max_iter:
            ...
            "adjust perturbation given the mode=['wf','shs']"
            if self.mode == 'wf':
                pert_x = utils_advBox.add_perturbation(X_input, noise_total_overshoot)
            elif self.mode == 'shs':
...
class LinfPGDAttack(object):
    def __init__(self, mode, x_box_min=-1,x_box_max=0,model=None, epsilon=0.3, k=40, a=0.01,
        random_start=True):
        """
        Attack parameter initialization. The attack performs k steps of
        size a, while always staying within epsilon from the initial
    ...
    def perturbation(self, X_nat, y):
        ...
        for i in range(self.k):
            ...
            "adjust perturbation given the mode=['wf','shs']"
            if self.mode == 'wf':
                X = utils_advBox.add_perturbation(X_var, pert)
            elif self.mode == 'shs':
                X += pert
            else:
                print('mode should in ["wf","shs"], system will exit.')
